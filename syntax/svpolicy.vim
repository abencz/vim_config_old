...
syn match svArgName		"'\?[a-zA-Z0-9\[\]${}._-]*'\?\s*:"
syn match svCursor		"'\?[a-zA-Z0-9\[\]${}._-]*'\?\s*:\s*cursor"

syn include @Tcl syntax/tcl_embeded.vim
syn region svComment    start="^\s*\#" keepend end="$" contains=svTodo
syn match svPCLStart	"^\s*\#!"
syn region svPCLLine	start="^\s*\#!" keepend end="$" contains=@Tcl,svPCLStart
syn region svString     start=+L\="+ skip=+\\\\\|\\"\[\]+ end=+"+
syn region svPCLCall    start="\$\@<=\[" keepend end="]\$\@=" contains=@Tcl
...
"hi link svPCLCall       Statement
"hi link svPCLLine		Statement
hi link svPCLStart		Statement
hi link svArgName		String
hi link svCursor		Type

hi link svAction		Function
